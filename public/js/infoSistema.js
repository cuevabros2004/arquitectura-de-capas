...
fetch('/info', {method:'GET'})
  ...
  .then(data =>    {    
    ...
    document.getElementById('infoSistema').insertAdjacentHTML('beforeend',  `<h4>Número de procesadores presentes en el servidor:</h4>`);
    document.getElementById('infoSistema').insertAdjacentHTML('beforeend',  `<td>  ${data["CPUs"].length}</td> `);
    }
    )
